module RoutesHelper
  ...
  def shortest_path(start, locations, dest, mode)
    ...
    if result.has_key? 'Error' or result['status'] != 'OK'
	  	...
	  	routes = []
	  	first_step = {geocode: geocode_to_s(result.first.start_location), departtime:______,  }
  		for leg in result.legs
  			departtime = route.last[:arrivetime]
  			routes << {geocode: geocode_to_s(leg.end_location), departtime: departtime, arrivetime: departtime + leg.duration["value"] }
